...
void runPhantomSweep(int stepDelayMs, uint8_t effectNum) {
  ...
  for (int ch = 0; ch < 4; ch++) {
    ...
    drvArray[ch].setWaveform(0, effectNum);
    drvArray[ch].setWaveform(1, 0); // End of sequence
    drvArray[ch].go();
    delay(stepDelayMs);
  }
}
...
void loop() {
        ...
        for (JsonObject step : steps) {
          ...
          if (ch >= 0 && ch < NUM_CHANNELS) {
            ...
            if (strcmp(type, "effect") == 0) {
              ...
              drvArray[ch].setMode(DRV2605_MODE_INTTRIG);
              drvArray[ch].setWaveform(0, effect);
              drvArray[ch].setWaveform(1, 0);
              drvArray[ch].go();
  ...
  if (Serial.available() > 0) {
    ...
    if (cmd == 's') {
      ...
      drvArray[targetCh].setWaveform(0, 47); // Pulse sharp click
      drvArray[targetCh].setWaveform(1, 0);
