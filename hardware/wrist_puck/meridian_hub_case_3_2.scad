...
// --- Lid engraving ---
lid_text = "Ember & Chris";
lid_text_size = 9;    // cap height, mm — tune to taste
lid_text_depth = 1;   // how far the text stands proud of the lid, mm
lid_text_font = "Liberation Sans:style=Bold"; // widely available; swap for any installed bold sans

// --- What to build this run ---
part = "lid"; // "lid" | "base" | "both" — see LAYOUT at the bottom

// Corner mounting holes in the lid — off for now, see note on lid() below.
has_lid_holes = false;
...
module base() {
    difference() {
        ...
        // channel cable ports, split across both walls to match the
        // mux board's connector edges (see photo — connectors run
        // along both the top and bottom edges of the board)
        cable_slots(cable_port_count_back, "back", board_bay_x0 + 6);
        cable_slots(cable_port_count_front, "front", board_bay_x0 + 6);
    }
...
module lid() {
    module lid_slab() {
        union() {
            ...
            // raised (embossed) text, centered on the lid's top face
            translate([case_l / 2, case_w / 2, wall])
    ...
    // Corner mounting holes disabled for now — insert_boss_od and
    // insert_hole_d aren't defined in this file (pulled out along with
    // the rest of the base params). Set has_lid_holes = true above and
    // restore both values once they're back in place.
    if (has_lid_holes) {
        difference() {
            lid_slab();
            for (x = [insert_boss_od, case_l - insert_boss_od])
...
// ---------------- LAYOUT ----------------
// part = "lid"  -> just the lid, for this print run
// part = "base" -> just the base (needs usbc_z/switch_w/etc. restored first)
// part = "both" -> both, side by side

if (part == "lid" || part == "both")
    translate([0, part == "both" ? case_w + 10 : 0, 0])
        lid();
