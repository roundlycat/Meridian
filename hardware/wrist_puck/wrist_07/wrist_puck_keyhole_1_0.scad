...
CHAMBER_D = 52;   // round chamber outer diameter -- grown from 48mm
                    // purely for servo margin/sweep comfort, not to
                    // also fit the LRA (verified: that would need
                    // 58mm+, past the 53mm wrist width entirely)
CHAMBER_R = CHAMBER_D / 2;

RECT_L = 66;    // rectangular body outer length (along arm) --
                 // extended from 48mm to fit a dedicated LRA zone at
                 // the far end (see LRA_CX/CY below), since neither
                 // the chamber nor the original rectangle had room
                 // for it once the servo was safely centered
...
// FIXED: was (-2, 4), a rough guess that put one corner of the
// servo's bounding box 25.05mm from chamber center against only
// 21.8mm of available wall -- a real breach, verified numerically
// (check_servo_clearance.scad), not caught until Sean questioned it.
// Dead-center is the only placement that clears all four corners --
// margin is 1.07mm, tight but real (verified: check_servo_centered.scad).
// Rotating the servo doesn't help on its own -- the bounding box's
// diagonal is the same regardless of which way it's rotated; only
// the CENTER position determines whether the corners clear the wall.
SERVO_CX = 0;
SERVO_CY = 0;
...
LRA_H    = 3.5;
// FIXED: was (2, -16), which collided with the servo once it was
// safely centered, and no other spot existed in either the chamber
// or the original (unextended) rectangle -- verified by numerical
// search, not assumed. Moved to a dedicated zone at the rectangle's
// far end, away from the servo, power stack, and compute stack.
// Sean confirmed physical proximity to the servo doesn't matter for
// how the haptic signal is perceived.
LRA_CX   = 66;
LRA_CY   = 0;
...
// FIXED: was "RECT_X0 + RECT_L/2", which re-centers on the
// rectangle's length -- meant that extending RECT_L to make room
// for the LRA zone also shifted this zone further out, colliding
// with the new LRA position. Pinned to its original, already-
// verified value (from when RECT_L was 48) so growing the rectangle
// only adds new space at the far end, rather than also moving
// existing, already-checked geometry.
PWR_CX = 34;
