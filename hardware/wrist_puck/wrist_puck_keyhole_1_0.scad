...
module bottom_shell() {
    ...
    difference() {
        ...
        // The real bug on the first two passes: each cavity only
        // extended to its OWN component height + a small overshoot,
        // which fell well short of shell_h (sized for the TALLEST
        // cavity). Every cavity was therefore a fully enclosed void
        // sealed on all sides -- no way to insert a battery into a
        // bubble with no opening. This wasn't a CGAL quirk; the
        // "12+12 disconnected triangles" from the connectivity
        // checker was CGAL correctly reporting a hollow-with-trapped-
        // void shape (two genuinely disjoint closed surfaces) -- the
        // checker and CGAL were both right, the geometry was wrong.
        // Fixed by overshooting every cavity's top past shell_h.

        // Servo + arm pocket
        translate([SERVO_CX, SERVO_CY, (FLOOR_T - 0.2 + shell_h + 1)/2])
            cube([SERVO_W, SERVO_D, (shell_h + 1) - (FLOOR_T - 0.2)], center = true);

        // LRA floor pocket (bare skin contact) -- opens through the
        // BOTTOM, not the top, so this one is correct as a short cavity
        translate([LRA_CX, LRA_CY, -0.2])
            cylinder(h = LRA_H + 0.2, d = LRA_DIAM);

        // Power stack cavity
        translate([PWR_CX, PWR_CY, (FLOOR_T - 0.2 + shell_h + 1)/2])
            cube([BATT_D, BATT_W, (shell_h + 1) - (FLOOR_T - 0.2)], center = true);

        // Compute stack cavity
        translate([CPU_CX, CPU_CY, (FLOOR_T - 0.2 + shell_h + 1)/2])
            cube([XIAO_W, XIAO_D, (shell_h + 1) - (FLOOR_T - 0.2)], center = true);
    }
}
