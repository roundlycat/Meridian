...
// ── MOUNT-PIECE REGISTRATION KEY (new in Rev 5.0) ────────────
// Small tongue on each ±X wall that keys into a matching slot cut
// into the wrist-mount piece (wrist_mount_piece.scad). Purely
// positional -- it does NOT carry strap tension (the mount pieces
// handle that via the captured M2 rod) -- it just keeps the puck
// from sliding or dropping out of the band when the strap is loose
// or off-wrist.
//
// Shared reference convention with the mount piece file: Z=0 here
// is the puck's exterior skin-facing plane -- the same plane the
// mount piece calls y=0 in its own sag/height axis (see z_bottom()
// there, and back_z(0) which is the mount piece's puck-facing
// face). KEY_Z is deliberately low on the puck to land within that
// face's z-bottom(0)..shell_top_z(0) band (~0 to ~3.3mm) rather
// than mid-height.
//
// This is a first-pass geometric key for dry-fit, not a precision
// joint -- the two files were parameterized independently with no
// shared coordinate system, so treat KEY_Z / KEY_CY as a starting
// point to true up by hand once both parts are printed, same
// spirit as the PB nub preload note above ("forgives tolerance
// stack-up" rather than trusting the number to be perfect).
//
// Sizing constraint: the mount piece is thin right at its
// puck-facing edge (~3.3mm total = pocket_depth + back_thickness,
// and only rim_width=2.5mm of solid material before the foam
// pocket starts). KEY_T/KEY_Z fit inside that ~3.3mm band with
// margin, and KEY_H fits inside the 2.5mm rim before it'd cut
// into the pocket -- see wrist_mount_piece.scad for the matching
// slot.
KEY_W  = 8.0;   // along the puck's Y (= mount piece's width/Z axis)
KEY_T  = 2.0;   // vertical extent (puck Z = mount piece's y/sag axis)
KEY_H  = 1.8;   // protrusion depth, outward from the puck wall
KEY_CY = 0;     // centered on the wall for now
KEY_Z  = 1.8;   // low on the puck, near the skin-facing plane
...
module bottom_shell() {
    difference() {
        union() {
            ...
            // Registration key tongues (±X) -- see the KEY_* param
            // block above for the shared-plane convention with the
            // mount piece file. Overlapped 0.3mm into the cylinder
            // wall rather than sitting tangent to it -- a flat box
            // face just touching a curved surface is a knife-edge
            // contact, not a solid union (same class of fix as the
            // M2 standoff posts above).
            KEY_OVERLAP = 0.3;
            for (s = [-1, 1])
                translate([s*(PUCK_D/2 + KEY_H/2 - KEY_OVERLAP), KEY_CY, KEY_Z])
                    cube([KEY_H, KEY_W, KEY_T], center=true);
        }
