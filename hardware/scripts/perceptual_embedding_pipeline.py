...
import asyncpg
import numpy as np
from openai import AsyncOpenAI  # or swap for local model client
...
class FeaturePacker:
    ...
    def _render_text(
        ...
        if domain == SensorDomain.ENVIRONMENTAL_FIELD:
            ...
            modality = "piezo" if "tap_count" in channels else "proximity"
            proximity = channels.get("proximity_mm", 0)
...
class Embedder:
    ...
    def __init__(
        ...
        self._client       = AsyncOpenAI() if not use_local else None
