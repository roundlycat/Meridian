// LD2410B + BME280 Presence Node Housing v0.11
...
// --- Rain hood bracing gussets (NEW in v0.11) ---
// The hood itself is an 18mm unsupported cantilever once past
// its wall anchor — fine for connectivity, bad for printing
// without support (confirmed: needed supports, which filled the
// whole cavity beneath it since the hood sits mid-height). These
// two ribs run from the wall up to the hood's underside as a
// self-supporting ramp (same printable-wedge principle as the
// lid's taper), positioned at x = ±30 — well outside the sensor
// window's x-range (±20.5) so they can't ever shadow it,
// regardless of z. A third, central rib was considered and
// rejected: at x=0 its base would pass within ~1mm of the
// window's edge — too thin a margin given how many times razor-
// thin margins have bitten this file already.
module rain_hood_gussets() {
    gusset_thick = 3;
    rib_x = [-30, 30];
    base_y = 21;   // overlaps the front wall band (20.1-22.5)
    base_z = -2;
    tip_y  = 38;   // inboard of the hood's front edge (40.5)
    tip_z  = 16;   // brackets the hood's underside either way
                    // the 8-degree tilt leans

    for (rx = rib_x)
        translate([rx, 0, 0])
            hull() {
                translate([0, base_y, base_z])
                    cube([gusset_thick, 4, 4], center=true);
                translate([0, tip_y, tip_z])
                    cube([gusset_thick, 4, 6], center=true);
            }
}
// v0.10 fix: the shelf's depth used to exactly equal the
// interior depth (40.2mm) — a zero-gap touch with the front and
// back walls rather than real overlap. The connectivity checker
// confirmed this *probably* renders fine, but it's the same risky
// pattern as the lid_boss 0.1mm gap from a few versions back, so
// it gets the same real-margin treatment now (0.6mm overlap each
// side) rather than relying on exact coincidence.
module ld2410b_mount() {
    shelf_z = body_height/6;
    translate([0, 0, shelf_z - ld_thick/2])
        cube([ld_len + 8, body_depth - 2*wall + 1.2, 2], center=true);
    for (side = [-1, 1])
...
module battery_bay() {
    ...
    for (s = [-1, 1])
        translate([s*(bat_len/2 + 1), 0, floor_z + 0.8 + bat_thick/2])
