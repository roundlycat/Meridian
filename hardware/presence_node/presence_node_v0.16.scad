// LD2410B + BME280 Presence Node Housing v0.16
...
//
// v0.16 fix: that clearance fix had a second side I didn't check
// — thinning the rail to 1.5mm to make room for it left a tall
// (11mm), thin, unbraced fin that snapped during printing. The
// rail only needs to be tall enough to block the battery's end
// from sliding past, not as tall as the battery itself, so the
// height comes down to 6mm instead — roughly halves the bending
// stress at the base without giving back any of the clearance
// fix, since this only changes Z, not the X/Y dimensions already
// verified.
module battery_bay() {
    floor_z = -body_height/2 + wall + 1;
    rail_width = 1.5;
    rail_height = 6;       // was bat_thick (11) — taller than needed
    ...
    for (s = [-1, 1])
        translate([s*rail_x, 0, floor_z + 0.8 + rail_height/2])
            cube([rail_width, bat_wid + 4, rail_height], center=true);
}
